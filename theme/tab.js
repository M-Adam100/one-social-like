function openTab(targetName, subContent) {
	...
	for (i = 0; i < tabLink.length; i++) {
		if (subContent) {
			if (tabLink[i].getAttribute("target-name") === targetName && tabLink[i].getAttribute("sub-target-name") === subContent) tabLink[i].classList.add("active");
...
(function () {
	...
	for (var i = 0; i < tabLink.length; i++) {
		tabLink[i].addEventListener("click", function () {
			if (this.getAttribute("sub-target-name")) openTab(this.getAttribute("target-name"), this.getAttribute("sub-target-name"));
			else openTab(this.getAttribute("target-name"));
		});
	}
})();